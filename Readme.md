...
**Operador `<<`** — escreve informações no arquivo, da mesma forma que o `cout` exibe no terminal.
```cpp
...
**`is_open()`** — verifica se o arquivo foi aberto corretamente. Recomendado usar antes de qualquer operação.
```cpp
if(arquivo.is_open()){
